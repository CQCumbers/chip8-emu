...
*/
            case 0xD000..<0xE000: // DRW Vx, Vy, nibble
                ...
                for row in 0..<Int(opcode & 0x000F) {
                    ...
                    for col in 0..<8 {
                        let screen_col = (Int(self.V[x]) - 1 + col) % 64
...
*/
        ...
        // Debug code
        //print(String(format:"pc: %2X", chip8.pc))
        //print(String(format:"opcode: %2X", opcode))
        //print(String(format:"I: %2X", chip8.I))
        //print(chip8.V)
        /*if (chip8.drawFlag == true) {
            for row in 0..<32 {
                for col in 0..<64 {
                    print(chip8.display[row*64 + col] ? "*" : " ", terminator:"")
                }
                print("|")
            }
...
final class DisplayView:UIImageView {
    let chip8:Chip8
    let width:Int
    let height:Int
...
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let pong_url = Bundle.main.url(forResource: "pong", withExtension: "rom")
        let pong = [UInt8] (try! Data(contentsOf: pong_url!))
        self.chip8 = Chip8(rom:pong)
        self.timer = Timer.scheduledTimer(timeInterval: 0.0017, target: self, selector: #selector(runCycle), userInfo: nil, repeats: true)
